	...
	else
	{
		...
		if((arg.w[0] >> 31) & 0x1 || ((arg.w[0] & 0x7FFFFFFF) == 0))
		{
			fwts_log_error(fw, "SMCCC_TEST_VERSION return value is invalid." 
				...
				arg.w[0]);
			//fwts_failed(fw, "Arm SMCCC_VERSION failed");
			return FWTS_ERROR;
		}
...
/*
 * smccc_arch_features_sbbr_check()
 * Asserts based on the BBR specification rules in 
 * Section 4.1. SMCCC architecture call requirements 
 * This function can be run only if SBBR flag is set
 * while running FWTS
 */

 static int smccc_arch_features_sbbr_check(fwts_framework *fw)
 {
	...
	fwts_log_info_verbatim(fw, "SMCCC SBBR check - the detected SMCCC version is %d.%d",
		smccc_major_version, smccc_minor_version);
	...
	{
		fwts_log_error(fw, "Arm SBBR check for ARM_SMCCC_ARCH_FEATURES failed");
		return FWTS_ERROR;
	}

	fwts_log_info_verbatim(fw, "Arm SBBR check for ARM_SMCCC_ARCH_FEATURES passed");
	return FWTS_OK;
 }
 ...
 */
 ...
 static int smccc_arch_features(fwts_framework *fw)
 {
	...
	//if --sbbr flag is set, additionally call the SBBR assertions function
	if (fw->flags & FWTS_FLAG_SBBR)
	{
		if (smccc_arch_features_sbbr_check(fw) == FWTS_ERROR)
		{
